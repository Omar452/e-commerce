<x-app-layout>
    <x-slot name="content">
        <x-title>Create a new item</x-title>
        <div class="mt-5 p-5 flex justify-center mx-auto border w-1/2 round-sm bg-blue-100">
            <form class="w-full" action="{{route('admin.items.store')}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="my-2 flex flex-col">
                    ...
                    <textarea class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" name="description" id="description" cols="30" rows="5">{{old('description')}}</textarea>
                    @error('description')
                    <div class="text-red-600 pl-1">
                        {{$message}}
                    </div>
                ...
                <div class="my-2 flex flex-col">
                    ...
                    <x-input class="@error('price') is-invalid @enderror" id="price" type="number" name="price" placeholder="Price in cents eg: 999, 1499" value="{{old('price')}}"/>
                    @error('price')
                    <div class="text-red-600 pl-1">
                        {{$message}}
                    </div>
                ...
                <div class="my-2 flex flex-col">
                    ...
                    <x-input class="@error('quantity') is-invalid @enderror" id="quantity" type="number" name="quantity" value="{{old('quantity')}}"/>
                    @error('quantity')
                    <div class="text-red-600 pl-1">
                        {{$message}}
                    </div>
